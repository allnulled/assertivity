...
```js
it(80).is(80); // returns true
it({a:{b:{c:10}}}); // stablishes the main reference
its("a","b","c").is(10); //returns true
report.as("This logs automatically the next assertion").that(90).is.number(); // returns true + logs the assertion
report.into("My Test > Part 1", "assertion 1").that(90).is.number(); // returns true + saves the assertion at report()["My Test"]["Part 1"]["assertion 1"]
report.into(" > assertion 2").that(91).is.number(); // returns true + saves the assertion at report()["My Test"]["Part 1"]["assertion 2"]
report.into(" > assertion 3").that(91).is.number(); // returns true + saves the assertion at report()["My Test"]["Part 1"]["assertion 3"]
```
Para un ejemplo más amplio, puedes consultar el test de la herramienta, aquí presente:
